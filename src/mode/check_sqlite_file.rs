...
pub fn check_sqlite_file(conn: &Connection, term: &Term) {
  let mut proxy_list = vec![];
  let mut success_proxy_list: Vec<String> = vec![];

  proxy_list = ger_proxy_from_sqlite(&conn, proxy_list);
          ...
          Ok(response) => {
              match response.kind {
                  ...
                  ActionTypes::JOB_SUCCESS => {
                      ...
                      let ip_only_string: Vec<&str> = response.payload.split("://").collect();

                      success_proxy_list.insert(success_proxy_list.len(), String::from(ip_only_string[1]));
